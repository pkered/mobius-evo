...
import { createJob, updateJob, createGenEvalParam } from '../../graphql/mutations';
import { uploadS3, listS3, getS3, downloadS3 } from '../../amplify-apis/userFiles';
import { UploadOutlined } from '@ant-design/icons';
import { AuthContext } from '../../Contexts';
import { Link } from 'react-router-dom';
import { Form, Input, InputNumber , Button, Steps,
    Table, Radio, Checkbox, Upload, message, Tag,
    Space, Spin, Row, Descriptions, Divider} from 'antd'
const { Step } = Steps;
const testDefault = {
    ...
    survival_size: 1,
    expiration: 3600,
    genFile_random_generated: 4,
    genFile_total_items: 4
};
...
function SettingsForm( { nextStep, formValuesState, jobType }) {
  ...
  async function initParams(jobID, jobSettings) {
    ...
    for (const genKey of jobSettings.genKeys) {
        let genFile;
        await downloadS3(`files/${genKey}`, data => {genFile = data}, ()=>{});
        ...
        for (let i = 0; i < jobSettings['genFile_' + genKey]; i++) {
            ...
            for (const param of params) {
                if (param.hasOwnProperty('step')) {
                    ...
                    paramset[param.name] = param.min + param.step * randomStep;
                } else {
                    paramset[param.name] = param.value;
                }
            }
            paramSet.params = JSON.stringify(paramset);
            allPromises.push(API.graphql(
                graphqlOperation(
                  createGenEvalParam,
                  {
                    input: paramSet}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Divider />
        <Form.Item
            label='Total Starting Items'
            name='genFile_total_items'>
